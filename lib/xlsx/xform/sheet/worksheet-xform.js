...
class WorkSheetXform extends BaseXform {
  ...
  prepare(model, options) {
    ...
    if (model.comments.length > 0) {
      ...
      model.comments.forEach(item => {
        ...
        const nrows =
          item.note.size && item.note.size.rows ? item.note.size.rows : item.note.texts.length + 2;
        const ncols = item.note.size && item.note.size.cols ? item.note.size.cols : 4;
        item.anchor = {
    ...
    // Form controls use VML only (no DrawingML drawing) and need ctrlProp relationships
    if (model.formControls && model.formControls.length > 0) {
      ...
      // Initialize formControlRefs array if not exists
      if (!options.formControlRefs) {
        options.formControlRefs = [];
      }
      ...
      model.formControls.forEach(control => {
        ...
        rels.push({
          ...
          Target: `../ctrlProps/ctrlProp${globalCtrlPropId}.xml`,
        });
        options.formControlRefs.push({
          id: globalCtrlPropId,
          model: control,
        });
      });

      // Add to vmlDrawings option for later rendering
      if (!options.vmlDrawings) {
        options.vmlDrawings = {};
      }
      const vmlKey = `../drawings/vmlDrawing${model.id}.vml`;
      if (!options.vmlDrawings[vmlKey]) {
        options.vmlDrawings[vmlKey] = {comments: [], formControls: []};
      }
      options.vmlDrawings[vmlKey].formControls = model.formControls;
    }
    ...
    (model.pivotTables || []).forEach(pivotTable => {
      // Handle both programmatic pivots (with tableNumber) and preserved pivots (with name)
      const target = pivotTable.tableNumber
        ? `../pivotTables/pivotTable${pivotTable.tableNumber}.xml`
        : `../pivotTables/${pivotTable.name}.xml`;
      rels.push({
        Id: nextRid(rels),
    ...
    // prepare preserved drawings (with charts)
    if (model.preservedDrawing) {
      model.drawing = {
        rId: nextRid(rels),
        name: model.preservedDrawing.name,
      };
      rels.push({
        Id: model.drawing.rId,
        Type: 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing',
        Target: `../drawings/${model.preservedDrawing.name}.xml`,
      });
    }
  ...
  render(xmlStream, model) {
    ...
    if (model.rels) {
      // add a <legacyDrawing /> node for VML (comments and/or form controls)
  ...
  reconcile(model, options) {
    ...
    if (model.drawing) {
      ...
      if (match) {
        ...
        // Check if this is a preserved drawing (with charts)
        const isPreserved = options.preservedDrawingsXml && options.preservedDrawingsXml[drawingName];

        if (isPreserved) {
          // Mark worksheet as having a preserved drawing
          model.preservedDrawing = {
            name: drawingName,
            rId: model.drawing.rId,
          };
        } else if (drawing && drawing.anchors) {
          // Only process if drawing was parsed (not preserved as raw XML for charts)
    ...
    // Preserve pivot tables from relationships
    if (options.preservedPivotTablesXml) {
      const pivotTableRels = (model.relationships || []).filter(
        rel => rel.Type === RelType.PivotTable
      );
      if (pivotTableRels.length > 0) {
        model.pivotTables = pivotTableRels.map(rel => {
          // rel.Target is like "../pivotTables/pivotTable1.xml"
          const match = rel.Target.match(/(pivotTable\d+)[.]xml/);
          const pivotTableName = match ? match[1] : null;
          return {
            name: pivotTableName,
            xml: options.preservedPivotTablesXml[pivotTableName],
            rels: options.preservedPivotTablesRels[pivotTableName],
          };
        });
      }
    }
