    ...
    switch (model.type) {
      ...
      case Enums.ValueType.Formula:
        if (model.result !== undefined && style && utils.isDateFmt(style.numFmt)) {
          ...
          // result is a numeric date serial. Coerce string to number first so
          // the date conversion runs. Non-numeric results (empty string, "N/A",
          // booleans) are left unchanged.
          let numericResult = model.result;
          if (typeof numericResult === 'string') {
            // Use Number() rather than parseFloat() so partial strings like
            // "2026-05-07" are rejected (Number returns NaN), not coerced to 2026.
            const parsed = Number(numericResult.trim());
