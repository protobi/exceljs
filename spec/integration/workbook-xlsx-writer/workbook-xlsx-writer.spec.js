...
describe('WorkbookWriter', () => {
  ...
  describe('Serialise', () => {
    ...
    it('with data bar conditional formatting using minimal options (#3015)', async () => {
      const options = {filename: TEST_XLSX_FILE_NAME, useStyles: true};
      const wb1 = new ExcelJS.stream.xlsx.WorkbookWriter(options);
      const ws1 = wb1.addWorksheet('DataBar Test');

      // Add sample data
      for (let i = 1; i <= 10; i++) {
        ws1.getCell(`A${i}`).value = i * 10;
      }

      // Add data bar with minimal options (should not crash)
      ...
      expect(ws2.conditionalFormattings).to.have.lengthOf(1);
      expect(ws2.conditionalFormattings[0].rules[0].type).to.equal('dataBar');
    });
  });
});
